...
# nullable enable
        ...
        public override void GenerateSquaresAttackedImproved(Board board, King opponentKing) {
            ulong pawnAttacks = MoveData.PawnAttacks[SquareIndex][(int)PieceTeam];
            board.AddAttacks(PieceTeam, pawnAttacks);
        }
        
        private ulong GenerateAttacks() {            
            Coordinate squareCoord = Board.ConvertSquareIndexToCoord(SquareIndex);
            ulong attacks = 0;
            
            ulong validRowForAttacks = BitboardHelper.BottomRowMask << ((squareCoord.y + (int)direction) * 8);
            ulong attackOneIndex = 1ul << (SquareIndex + 9 * (int)direction);
            ulong attackTwoIndex = 1ul << (SquareIndex + 7 * (int)direction);
            attacks |= (attackOneIndex | attackTwoIndex) & validRowForAttacks;
            return attacks;
        }
