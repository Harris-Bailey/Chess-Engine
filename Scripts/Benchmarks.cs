...
namespace BenchmarkDiagnoser {
    ...
    public class Benchmarks {
        ...
        // [Benchmark]
        public void GenerateSquaresAttackedStandard() {
            for (int i = 0; i < n; i++) {
                Piece queen = board.pieces[34];
                // queen.Reset();
                queen.GenerateSquaresAttackedImproved(board, board.GetTeamsKing(board.opponentTeam));
            }
        }
        ...
        public void GetAttacksUsingNormal() {
            for (int i = 0; i < n; i++) {
                foreach (Piece piece in board.pieces) {
                    if (piece == null || piece.PieceTeam == board.currentTeam)
                        continue;
                    piece.GenerateSquaresAttackedImproved(board, activeTeamsKing);
                }
            }
        }
